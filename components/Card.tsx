...
export const Card: React.FC<CardProps> = ({ card, onClick, className, disabled }) => {
    ...
    const getCornerSymbol = () => {
        const fill = "#ffffff";
        switch (card.color) {
            case 'red': return <path d="M50 85 C50 85 10 55 10 30 C10 10 35 10 50 30 C65 10 90 10 90 30 C90 55 50 85 50 85 Z" fill={fill} stroke={strokeColor} strokeWidth="5" strokeLinejoin="round" />; // Heart/Flame
            case 'blue': return <path d="M50 15 L85 55 A25 25 0 0 1 15 55 Z" fill={fill} stroke={strokeColor} strokeWidth="5" strokeLinejoin="round" />; // Drop
            case 'green': return <path d="M50 15 C80 15 90 45 50 85 C10 45 20 15 50 15 Z" fill={fill} stroke={strokeColor} strokeWidth="5" strokeLinejoin="round" />; // Leaf/Spade
            case 'yellow': return <polygon points="50,10 65,40 100,45 75,70 80,100 50,85 20,100 25,70 0,45 35,40" fill={fill} stroke={strokeColor} strokeWidth="5" strokeLinejoin="round" />; // Star
            case 'cyan': return <polygon points="50,10 80,50 50,90 20,50" fill={fill} stroke={strokeColor} strokeWidth="5" strokeLinejoin="round" />; // Diamond
            default: return <circle cx="50" cy="50" r="30" fill={fill} stroke={strokeColor} strokeWidth="5" />;
        }
    };
    ...
    const getCenterSVG = () => {
        ...
        const UnoOval = (
            <ellipse cx="50" cy="50" rx="42" ry="32" fill="#fdfbf7" stroke={strokeColor} strokeWidth="6" transform="rotate(-30 50 50)" />
        );
        ...
        if (card.type === 'number' || card.type === '+2' || parseInt(val as string)) {
            return (
                <g>
                    {UnoOval}
                    <ellipse cx="50" cy="50" rx="36" ry="26" fill={iconColor} stroke="none" transform="rotate(-30 50 50)" />
        ...
        if (card.type === 'skip') {
            return (
                <g>
                    {UnoOval}
                    <circle cx="50" cy="50" r="28" fill="none" stroke={strokeColor} strokeWidth="14" />
                    <circle cx="50" cy="50" r="28" fill="none" stroke={iconColor} strokeWidth="8" />
                    <line x1="30" y1="30" x2="70" y2="70" stroke={strokeColor} strokeWidth="14" strokeLinecap="round" />
                    <line x1="30" y1="30" x2="70" y2="70" stroke={iconColor} strokeWidth="8" strokeLinecap="round" />
                </g>
            )
        }
        ...
        return (
            <g>
                {UnoOval}
                <polygon points="50,20 80,75 20,75" fill={iconColor} stroke={strokeColor} strokeWidth="8" strokeLinejoin="round" />
